...
- Green으로 경로(Route) 전환
  ...
  이번에는 blue 서비스를 가리키는 외부 경로(Route)가 이미 있으므로 서비스를 노출하기 위해 `oc expose`를 실행할 필요가 없습니다. 이번에는 경로(Route)를 green 서비스로 전환합니다. 이를 수행하는 방법은 몇 가지가 있습니다. 먼저, 가장 쉬운 방법인 `oc edit` 명령을 사용하고 경로(Route)의 매니페스트에서 올바른 위치를 찾는 데 도움을 줍니다. 나중에 `oc patch` 명령을 사용하여 전환을 수행합니다. `oc patch` 명령을 사용하면 blue 배포와 green 배포 간 전환을 자동화하려는 경우에 유용합니다.
